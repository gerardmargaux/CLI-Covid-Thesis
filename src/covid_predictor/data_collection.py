...
try:
    ...
    google_trends_errors = (exceptions.ResponseError, ReadTimeout, ConnectTimeout)
    tor_ip_set = False

dir_daily = Path(__file__).parents[2]/"data/trends/collect_daily/"
dir_daily_gap = Path(__file__).parents[2]/"data/trends/collect_gap/"
dir_hourly = Path(__file__).parents[2]/"data/trends/collect/"
dir_min_hourly = Path(__file__).parents[2]/"data/trends/collect_minimal/hourly/"
dir_min_daily = Path(__file__).parents[2]/"data/trends/collect_minimal/daily/"
dir_model = Path(__file__).parents[2]/"data/trends/model/"
dir_explore = Path(__file__).parents[2]/"data/trends/explore/"
day_format = "%Y-%m-%d"
...
def actualize_hospi(european_geo=False):
    ...
    if not european_geo:
        ...
        filepath = Path(__file__).parents[2]/"data/hospi/be-covid-hospi.csv"
        df_hospi_be.to_csv(filepath, index=False)
        ...
        filepath = Path(__file__).parents[2]/"data/hospi/fr-covid-hospi-total.csv"
        df_hospi_fr.to_csv(filepath, index=False)
        ...
        filepath = Path(__file__).parents[2]/"data/hospi/fr-covid-hospi.csv"
        df_hospi_fr_new.to_csv(filepath, index=False)
        ...
        filepath = Path(__file__).parents[2]/"data/hospi/world.csv"
        df_world.drop(columns=['population']).to_csv(filepath, index=False)
...
def collect_data(method: str = 'daily', topics: Dict[str, str] = None, geo: Dict[str, str] = None, gap: bool = True):
    ...
    if topics is None:
        topics = list_topics
    if geo is None:
        geo = french_region_and_be

    if tor_ip_set:
        trends_request = TorTrendsRequest(max_errors=np.inf)  # use tor proxy for collecting data
    else:
        trends_request = LocalTrendsRequest(max_errors=np.inf)  # use local ip address for collecting data
...
def collect_minimal_data(topics: Dict[str, str] = None, geo: Dict[str, str] = None):
    if topics is None:
        topics = utils.list_topics
    if geo is None:
        geo = utils.french_region_and_be
...
def collect_relevant_topics(topics: Dict[str, str] = None, geo: Dict[str, str] = None):
    ...
    df_hospi = utils.hospi_french_region_and_be(url_hospi_france_tot, url_hospi_france_new, url_hospi_belgium,
                                               url_department_france, utils.french_region_and_be, new_hosp=True,
                                               tot_hosp=True)['BE']
